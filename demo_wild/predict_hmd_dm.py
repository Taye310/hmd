...
piclist = os.listdir(opt.dir)
for i in trange(39, len(piclist), desc='mask process'):
    opt.img = opt.dir + piclist[i]
    opt.outf = "/home/zhangtianyi/ShareFolder/data/hmd_masked/train/predict_hmr_result/" + opt.img.split("/")[-1][:-4] + "/"
    ...
    if opt.step:
        ...
        #for depth
        # PIL.Image.fromarray(depth_pred).save(opt.outf+"%04d_depth_pred.png" % test_num)
        # PIL.Image.fromarray(depth_mask).save(opt.outf+"%04d_depth_mask.png" % test_num)
        # PIL.Image.fromarray(depth_proj).save(opt.outf+"%04d_depth_proj.png" % test_num)
        np.save(opt.outf+"%04d_depth_pred.npy" % test_num, depth_pred)
        np.save(opt.outf+"%04d_depth_mask.npy" % test_num, depth_mask)
        np.save(opt.outf+"%04d_depth_proj.npy" % test_num, depth_proj)
